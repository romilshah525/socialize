...
<body>
				...
				<div class="col-lg-12">
					<div class="box"> 
						<div class="box-footer clearfix">
							<form action="uploadStatus?userId=${userId}" method="post">
								<button class="kafe-btn kafe-btn-mint-small pull-right btn-sm" type="submit">Upload</button>
								<ul class="nav nav-pills nav-sm">
									<li class="nav-item">
										<a class="nav-link" href="javascript:void(0);">
											<input type="hidden" role="uploadcare-uploader" data-crop="1:1" data-images-only/>
											
											<!-- <i id="camera_icon" class="fa fa-camera text-muted"></i> -->
										</a>
									</li>
								</ul>
								<br>
				                  ...
				                  -o-background-size: cover;">
											<div class="story-body text-center">
												<p></p>
												 	...
												 	<input id="status_selector" name="status_selector" type="hidden" style="display: none;">
												 	<input id="data_url" name="data_url" type="hidden" style="display: none;">
	...
	<section class="stories">
		<div class="container">
			<div class="row">
				...
				<c:forEach var="status" items="${getFriendsStatusForPageStories}">
					<c:set var="statusOfUser" value="${userService.getUserWithId( status.getUserId() )  }"/>
					<div class="col-lg-4">
						<a href="seeStory?userId=${userId}&statusId=${status.getId()}">
	                  ...
	                  -o-background-size: cover;">
								<div class="story-body text-center">
									<div class=""><img class="img-circle" src="data:image/jpg;base64,${ statusOfUser.getProfileImage() }" alt="user"></div>
				...
				<div class="col-lg-4">
					<a href="#">
						<div class="storybox" style="background: linear-gradient( rgba(34,34,34,0.78), rgba(34,34,34,0.78)), url('resources/assets/img/posts/9.jpg') no-repeat;
		          background-size: cover;
                  background-position: center center;
                  -webkit-background-size: cover;
                  -moz-background-size: cover;
				...
				<div class="cardbox-like">
					<ul>
						<c:if test="${prev > 0 }">
							<li><a href="stories?userId=${userId }&statusId=0&prev=${prev-1}&next=${next-1}"> <span style="font-size: 50px;"> ${prev } &laquo;</span> </a></li>
						</c:if>
						<c:if test="${getFriendsStatusForPageStories.size() == 6 }">
							<li><a href="stories?userId=${userId }&statusId=0&prev=${prev+1}&next=${next+1}"> <span style="font-size: 50px;"> &raquo; ${next+1 }</span> </a> </li>
						</c:if>
					</ul>
				</div>
			</div>
	...
	<script>
		...
		widget.onUploadComplete((fileInfo) => {
	    	  ...
	    	  toDataURL(fileInfo.cdnUrl, function(dataUrl) {
  			  console.log('RESULT:', dataUrl);
  			  preview.style.backgroundImage = "url("+dataUrl+")";//"url('img_tree.png')";
					preview.style.height= "300px";
					preview.classList.add("storybox");
					
					status_selector.setAttribute("value",fileInfo.name);
					data_url.setAttribute("value", dataUrl);
  			})
