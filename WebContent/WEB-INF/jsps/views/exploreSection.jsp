	...
	<section class="newsfeed">
		<div class="container">
			...
			<div class="row one-row">
	    		<div class="col-lg-12">
	    			<a href="exploreFriends?userId=${userId}"><h4>See All</h4></a>
				</div>
	  		 </div>
	   
			<div class="row top-row">
				<c:forEach items="${recommendedFriends}" var="recomendedFriend">
							...
							<div class="tr-post">
								<div class="entry-header">
									<div class="entry-thumbnail">
										<a href="profile?userId=${userId}&profileId=${recomendedFriend.getId()}">
											<c:set var="latestPost"  value="${userService.getLatestPostByUser(recomendedFriend.getId()) }"/>
											<c:if test="${latestPost.getPost()==null}">
												<img class="img-fluid" src="data:image/jpg;base64,${recomendedFriend.getProfileImage()}" alt="Image">
											</c:if>
											<c:if test="${latestPost.getPost()!=null}">
												<img class="img-fluid" src="data:image/jpg;base64,${latestPost.getPost()}" alt="Image">
											</c:if>
										</a>
									</div>
								</div>
								<div class="post-content">
									<div class="author-post text-center">
										<a href="profile?userId=${userId}&profileId=${recomendedFriend.getId()}"><img class="img-fluid rounded-circle"
												src="data:image/jpg;base64,${recomendedFriend.getProfileImage()}" alt="Image"></a>
									</div>
									<div class="card-content">
										<a href="profile?userId=${userId}&profileId=${recomendedFriend.getId()}">
											<h4>${recomendedFriend.getName()}</h4>
										</a>
										<span>${recomendedFriend.getEmail()}</span>
									</div>
									<a href="followFromExplore?userId=${userId}&requestPersonId=${recomendedFriend.getId()}" class="kafe-btn kafe-btn-mint-small full-width">
										Follow
									</a>
								</div>
							</div>
			...
			<div class="row">
				...
				<c:forEach items="${randomPicturesOfUserWhichAreNotFriend}" var="randomPost">
					
					<div class="col-lg-4">
						<a href="seePost?userId=${userId}&postId=${randomPost.getId()}&profileId=${randomPost.getId()}" data-toggle="modal">
							<div class="explorebox" style="background: linear-gradient( rgba(34,34,34,0.2), rgba(34,34,34,0.2)), url('data:image/jpg;base64,${randomPost.getPost()}') no-repeat;
	                  ...
	                  -o-background-size: cover;">
								<div class="explore-top">
									<div class="explore-like"><i class="fa fa-heart"></i> <span>${randomPost.getLikes().size() }</span></div>
									<div class="explore-circle pull-right"><i class="far fa-bookmark"></i></div>
								</div>
								<div class="explore-body">
									<div class=""><img class="img-circle" src="data:image/jpg;base64,${ userService.getProfileImageOfUser( randomPost.getUserId() ) }" alt="user"></div>
								</div>
							</div>
						</a>
					</div>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-md-8 modal-image">
							<img class="img-responsive" src="resources/assets/img/posts/6.jpg" alt="Image" />
						</div>
